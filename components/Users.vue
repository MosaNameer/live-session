        ...
        <div flex="~" justify="between">
            ...
            <div v-if="store.isAdmin && user.id !== userId" text="sm" flex="~ gap-2" items="center">
                ...
                <div @click="() => {}" text-whitesec text-center flex="~" cursor="pointer" v-if="store.getCurrentSlide?.type == 'Question'">
                    ...
                    <span>
                        (
                            {{store.getCorrectQuestionsByUserId(user.id)?.filter(q => q.result)?.length}} <Icon name="ion:checkmark-round" text="success"/>
                            and {{store.getCorrectQuestionsByUserId(user.id)?.filter(q => q.type=="text")?.length}} <Icon name="carbon:face-pending" text="whitesec"/>
                        )
                    </span>
                </div>
            </div>
